...
const ProjectCard = ({ project }: { project: Project }): JSX.Element => {
    ...
    <div
        ...
        onClick={toggle}
        className="flex w-150 cursor-pointer flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-xl focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cyan-600"
    >
        ...
        <div className="flex min-h-45 flex-col justify-between gap-4 p-6">
            ...
            <div className="flex items-start justify-between gap-3">
                <div className="flex flex-col gap-1">
                    <p className="text-xl font-bold tracking-tight text-slate-900"> {project.title} </p>
                    <p className="text-sm italic text-slate-500"> {project.skills} </p>
                </div>
            ...
            <div className={`grid transition-[grid-template-rows] duration-500 ease-out ${isOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}>
                <div className="flex min-h-0 flex-col gap-4 overflow-hidden">
                    ...
                    ? <div className="flex flex-col gap-2">
                        <img
                            className="aspect-video w-full rounded-lg border border-slate-200 bg-slate-100 object-contain"
                        ...
                        ? <div className="flex flex-row items-center justify-center gap-4" onClick={event => event.stopPropagation()}>
                            <button
                                ...
                                className="flex h-7 w-7 cursor-pointer items-center justify-center rounded-full border border-slate-200 text-slate-500 transition hover:bg-slate-100 hover:text-slate-800"
                            >
                                ‹
                            </button>
                            <span className="text-xs tabular-nums text-slate-500"> {imageIndex + 1} / {imageCount} </span>
                            <button
                                type="button"
                                onClick={nextImage}
                    ...
                    {/* Bullet points */}
                    <ul className="flex list-disc flex-col gap-2 pl-5 text-sm leading-relaxed text-slate-700 marker:text-cyan-500">
                    ...
                    ? <div className="flex flex-col gap-2 border-t border-slate-200 pt-4">
                        <p className="text-xs font-semibold uppercase tracking-wide text-slate-400"> Collaborators </p>
                        <div className="flex flex-col gap-1">
                            {project.collaborators.map((person, personIndex) => (
                                <span key={personIndex} className="text-sm text-slate-600"> {person} </span>
                            ))}
                        </div>
                      </div>
                    : null}
            ...
            <div className="flex flex-row flex-wrap gap-2">
                {project.tags.map((tag, tagIndex) => (
                    <span key={tagIndex} className="rounded-full bg-cyan-50 px-3 py-1 text-xs font-medium text-cyan-800 ring-1 ring-cyan-200 ring-inset"> {tag} </span>
                ))}
            </div>
...
<section id="projects">
    ...
    <div className="mb-10 flex flex-col items-center gap-3">
        ...
        <p className="max-w-xl text-center text-lg leading-relaxed text-slate-500">
